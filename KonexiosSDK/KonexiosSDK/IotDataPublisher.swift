...
public class IotDataPublisher: NSObject, EDQueueDelegate {
    ...
    public func queue(_ queue: EDQueue!, processJob job: [AnyHashable : Any]!, completion block: KonexiosSDK.EDQueueCompletionBlock!) {
    ...
    private func doSendData(dataLoad: IotDataLoad, completion block: KonexiosSDK.EDQueueCompletionBlock!) {
        ...
        case .IotConnect:
            ...
            if !KonexiosIot.sharedInstance.isMQTTConnected() {
                KonexiosIot.sharedInstance.reconnectMQTT()
            }
            KonexiosIot.sharedInstance.sendTelemetries(data: dataLoad) { (success) -> Void in
                if block != nil {
                    if (success) {
                        block(EDQueueResult.success)
                    } else {
